문제
소희는 공개키와 개인키 한 쌍으로 보안을 유지하는 것이 매우 불편하다고 생각했다. 그래서 소희는 공개키만을 이용하는 암호화 체계를 개발했다. 
이를 "철벽 보안 알고리즘"이라고 부르기로 했다. 알고리즘은 다음과 같다.
...
int main() {
	...
	while (T--) {
		...
		string s;
		for (int i = 0; i < n; i++) {   // 단어들이 들어오면 Map에 저장
			cin >> s;
			M[s] = i;
		}
		for (int i = 0; i < n; i++) {     // 단어들의 재조합이 들어오면 어떤 순서로 재배열되었는지 저장 (1 -> 2) 
			cin >> s;
			pat.push_back(make_pair(i, M[s])); // (제2키, 제1키) --> 2키를 1키 위치로 넣어야 한다
		}
		for (int i = 0; i < n; i++) {   //암호문 저장
			cin >> s;
			v1.push_back(s);
		}
		v2.resize(v1.size());   //평문으로 만들어줄 새로운 벡터
		for (int i = 0; i < n; i++) {  // 재조합 방법이 들어간 pat 벡터 탐색하면서 암호문 복구
			v2[pat[i].second] = v1[pat[i].first];  // 1 -> 2로 변형되었으면 v2[1] = v1[2];  <-- 2 idx를 1idx로 넣아야함
		}
		for (int i = 0; i < n; i++) {
			cout << v2[i] << " ";
		}
