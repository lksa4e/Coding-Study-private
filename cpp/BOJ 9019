...
void bfs() {
	...
	while (!q.empty()) {
		...
		if (!visit[next_d]) {
			visit[next_d] = 1;
			q.push({ next_d, cur_str + "D" });
		}
		int next_s = cur - 1 >= 0 ? cur - 1 : 9999;
		if (!visit[next_s]) {
			visit[next_s] = 1;
			q.push({ next_s, cur_str + "S" });
		}
		int next_l = (cur % 1000) * 10 + (cur / 1000);
		if (!visit[next_l]) {
			visit[next_l] = 1;
			q.push({ next_l, cur_str + "L" });
		}
		int next_r = (cur / 10) + (cur % 10) * 1000;
		if (!visit[next_r]) {
			visit[next_r] = 1;
			q.push({ next_r, cur_str + "R" });
		}
	}
}
int main() {
